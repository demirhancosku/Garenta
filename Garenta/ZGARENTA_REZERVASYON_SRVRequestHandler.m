...
NSString * const kLoadET_KK_RETURNSetForReservationServiceCompletedNotification = @"LoadET_KK_RETURNSetForReservationServiceCompleted";
...
- (void)loadET_KK_RETURNSetForReservationServiceCompleted:(id <Requesting>)request
{
    NSMutableDictionary *userInfoDict = [NSMutableDictionary dictionaryWithDictionary:request.userInfo];
    NSError *error;
    ReservationService *parentReservationServiceItem = request.userInfo[kResponseParentItem];
    [parentReservationServiceItem loadET_KK_RETURNSetWithData:request.responseData error:&error];
    if (error) {
    	userInfoDict[kParsingError] = error;
    }
    else {
    	userInfoDict[kResponseItems] = parentReservationServiceItem.ET_KK_RETURNSet;
    }
    [[NSNotificationCenter defaultCenter] postNotificationName:kLoadET_KK_RETURNSetForReservationServiceCompletedNotification object:self userInfo:userInfoDict];   
}

- (void)loadET_KK_RETURNSetForReservationService:(ReservationService *)aReservationService
{
    ODataQuery *query = aReservationService.ET_KK_RETURNSetQuery;
    NSDictionary *userInfoDict = @{kResponseParentItem : aReservationService , kRequestUserInfoCompletedNotification : kLoadET_KK_RETURNSetForReservationServiceCompletedNotification}; 
    [connectivityHelper executeBasicAsyncRequestWithQuery:query andRequestDelegate:self andDidFinishSelector:@selector(loadET_KK_RETURNSetForReservationServiceCompleted:) andUserInfo:userInfoDict];
}
...
- (void)createReservationServiceCompleted:(id <Requesting>)request
{
    ...
    if ([responseData length] > 0) {
        ...
        item = [ReservationService parseExpandedReservationServiceEntryWithData:responseData andServiceDocument:service.sdmServiceDocument error:&error];
...
- (void)createET_KK_RETURN:(ET_KK_RETURN *)aNewET_KK_RETURN forReservationService:(ReservationService *)aReservationService
{
	[self loadCSRFData];
    NSError *error = nil;
    NSString *payload = nil;
    if (self.useJSON) {
        payload = [service getJSONForCreateRequest:aNewET_KK_RETURN error:&error];
    }
    else {
        payload = [service getXMLForCreateRequest:aNewET_KK_RETURN error:&error];
    }
	...
	if (error) {
        ...
        NSDictionary *userInfoDict = @{kResponseParentItem : aNewET_KK_RETURN , kRequestUserInfoCompletedNotification : kCreateET_KK_RETURNCompletedNotification};
    	[connectivityHelper executeCreateAsyncRequestWithQuery:aReservationService.ET_KK_RETURNSetQuery andBody:payload andCSRFData:csrfData andRequestDelegate:self andDidFinishSelector:@selector(createET_KK_RETURNCompleted:) andUserInfo:userInfoDict];
    }
}
