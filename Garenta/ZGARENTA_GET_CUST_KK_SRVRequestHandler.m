/*
 ...
 File: ZGARENTA_GET_CUST_KK_SRVRequestHandler.m
...
NSString * const kLoadCustKKSetCompletedNotification = @"LoadCustKKSetCompleted";
NSString * const kLoadCustKKServiceCompletedNotification = @"LoadCustKKServiceCompleted";
NSString * const kLoadET_CARDSSetForCustKKServiceCompletedNotification = @"LoadET_CARDSSetForCustKKServiceCompleted";
NSString * const kCreateCustKKServiceCompletedNotification = @"CreateCustKKServiceCompleted";
NSString * const kUpdateCustKKServiceCompletedNotification = @"UpdateCustKKServiceCompleted";
NSString * const kDeleteCustKKServiceCompletedNotification = @"DeleteCustKKServiceCompleted";

NSString * const kLoadET_CARDSetCompletedNotification = @"LoadET_CARDSetCompleted";
NSString * const kLoadET_CARDSCompletedNotification = @"LoadET_CARDSCompleted";
NSString * const kCreateET_CARDSCompletedNotification = @"CreateET_CARDSCompleted";
NSString * const kUpdateET_CARDSCompletedNotification = @"UpdateET_CARDSCompleted";
NSString * const kDeleteET_CARDSCompletedNotification = @"DeleteET_CARDSCompleted";
...
@implementation ZGARENTA_GET_CUST_KK_SRVRequestHandler
...
- (id)init
{
    self = [super init];
    if (self) { 
        service = [[ZGARENTA_GET_CUST_KK_SRVService alloc] init]; // This will be re-initiated with the updated metadata from the server (according to the user locale), in first login. See executeLoginWithUsername:andPassword:error method implementation.
...
{
    static ZGARENTA_GET_CUST_KK_SRVRequestHandler *instance;
	
    @synchronized(self) {
        if (!instance) {
            instance = [[ZGARENTA_GET_CUST_KK_SRVRequestHandler alloc] init];
        }
        return instance;
    }
}
    ...
    // In case local metadata is not used, negotiate the best matching service version, get the service data, the service metadata, and re-initialize the service proxy object
        ...
        if (serviceDocumentData) {
            ...
            if (serviceMetadataData) {
                service = [[ZGARENTA_GET_CUST_KK_SRVService alloc] initWithServiceDocument:serviceDocumentData andMedatadata:serviceMetadataData];
...
- (void)loadCustKKSetCompleted:(id <Requesting>)request
{
    NSDictionary *userInfoDict;
    NSError *error;
    NSMutableArray *items = [service getCustKKSetWithData:request.responseData error:&error];
    if (error) {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kLoadCustKKSetCompletedNotification object:self userInfo:userInfoDict];
}

- (void)loadCustKKSet
{
    ODataQuery *query = service.CustKKSetQuery;
    NSDictionary *userInfoDict = @{kRequestUserInfoCompletedNotification : kLoadCustKKSetCompletedNotification};
    [connectivityHelper executeBasicAsyncRequestWithQuery:query andRequestDelegate:self andDidFinishSelector:@selector(loadCustKKSetCompleted:) andUserInfo:userInfoDict];
}


- (void)loadCustKKServiceCompleted:(id <Requesting>)request
{
    NSMutableDictionary *userInfoDict = [NSMutableDictionary dictionaryWithDictionary:request.userInfo];
    NSError *error;
    CustKKService *item = [CustKKService parseExpandedCustKKServiceEntryWithData:request.responseData andServiceDocument:service.sdmServiceDocument error:&error];
    if (error) {
    	userInfoDict[kParsingError] = error;
    }
    else {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kLoadCustKKServiceCompletedNotification object:self userInfo:userInfoDict];
}

- (void)loadCustKKService:(CustKKService *)aCustKKService
{
    [self loadCustKKService:aCustKKService expand:NO];
}

- (void)loadCustKKService:(CustKKService *)aCustKKService expand:(BOOL)expand
{
//    ODataQuery *query = [[ODataQuery alloc] initWithURL:aCustKKService.baseUrl];
//    ODataQuery *query = [service getVersiyonServiceEntryQueryTypedWithIAppName:aVersionService.IAppName andIVers:aVersionService.IVers];
    ODataQuery *query = [service getCustKKSetEntryQueryTypedWithIKunnr:aCustKKService.IKunnr];
    if (expand) {
        [query expand:@"ET_CARDSSet"]; 
    }
    NSDictionary *userInfoDict = @{kResponseParentItem : aCustKKService , kRequestUserInfoCompletedNotification : kLoadCustKKServiceCompletedNotification};
    [connectivityHelper executeBasicAsyncRequestWithQuery:query andRequestDelegate:self andDidFinishSelector:@selector(loadCustKKServiceCompleted:) andUserInfo:userInfoDict];
}

- (void)loadET_CARDSSetForCustKKServiceCompleted:(id <Requesting>)request
{
    NSMutableDictionary *userInfoDict = [NSMutableDictionary dictionaryWithDictionary:request.userInfo];
    NSError *error;
    CustKKService *parentCustKKServiceItem = request.userInfo[kResponseParentItem];
    [parentCustKKServiceItem loadET_CARDSSetWithData:request.responseData error:&error];
    if (error) {
    	userInfoDict[kParsingError] = error;
    }
    else {
    	userInfoDict[kResponseItems] = parentCustKKServiceItem.ET_CARDSSet;
    }
    [[NSNotificationCenter defaultCenter] postNotificationName:kLoadET_CARDSSetForCustKKServiceCompletedNotification object:self userInfo:userInfoDict];   
}

- (void)loadET_CARDSSetForCustKKService:(CustKKService *)aCustKKService
{
    ODataQuery *query = aCustKKService.ET_CARDSSetQuery;
    NSDictionary *userInfoDict = @{kResponseParentItem : aCustKKService , kRequestUserInfoCompletedNotification : kLoadET_CARDSSetForCustKKServiceCompletedNotification}; 
    [connectivityHelper executeBasicAsyncRequestWithQuery:query andRequestDelegate:self andDidFinishSelector:@selector(loadET_CARDSSetForCustKKServiceCompleted:) andUserInfo:userInfoDict];
}

#pragma mark Create

- (void)createCustKKServiceCompleted:(id <Requesting>)request
{
    NSMutableDictionary *userInfoDict = [NSMutableDictionary dictionaryWithDictionary:request.userInfo];
    NSData *responseData = request.responseData;
    if ([responseData length] > 0) {
        NSError *error;
        CustKKService *item = [CustKKService parseCustKKServiceEntryWithData:responseData error:&error];
        if (error) {
        	userInfoDict[kParsingError] = error;
        }
        else{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kCreateCustKKServiceCompletedNotification object:self userInfo:userInfoDict];
...
- (void)createCustKKService:(CustKKService *)aCustKKService
{
    ...
    if (self.useJSON) {
        payload = [service getJSONForCreateRequest:aCustKKService error:&error];
    }
    else {
        payload = [service getXMLForCreateRequest:aCustKKService error:&error];
    }
	...
	if (error) {
        ...
        NSDictionary *userInfoDict = @{kResponseParentItem : aCustKKService , kRequestUserInfoCompletedNotification : kCreateCustKKServiceCompletedNotification};
    	[connectivityHelper executeCreateAsyncRequestWithQuery:service.CustKKSetQuery andBody:payload andCSRFData:csrfData andRequestDelegate:self andDidFinishSelector:@selector(createCustKKServiceCompleted:) andUserInfo:userInfoDict];
    }
}
...
- (void)createET_CARDS:(ET_CARDS *)aNewET_CARDS forCustKKService:(CustKKService *)aCustKKService
{
    ...
    NSString *payload = nil;
    if (self.useJSON) {
        payload = [service getJSONForCreateRequest:aNewET_CARDS error:&error];
    }
    else {
        payload = [service getXMLForCreateRequest:aNewET_CARDS error:&error];
    }
    
	if (error) {
        NSMutableDictionary *userInfoDict = [@{} mutableCopy];
        userInfoDict[kParsingError] = error;
        [[NSNotificationCenter defaultCenter] postNotificationName:kCreateET_CARDSCompletedNotification object:self userInfo:userInfoDict];
    }
    else {
        NSDictionary *userInfoDict = @{kResponseParentItem : aNewET_CARDS , kRequestUserInfoCompletedNotification : kCreateET_CARDSCompletedNotification};
    	[connectivityHelper executeCreateAsyncRequestWithQuery:aCustKKService.ET_CARDSSetQuery andBody:payload andCSRFData:csrfData andRequestDelegate:self andDidFinishSelector:@selector(createET_CARDSCompleted:) andUserInfo:userInfoDict];
    }
}

#pragma mark Update

- (void)updateCustKKServiceCompleted:(id <Requesting>)request
{
    NSMutableDictionary *userInfoDict = [NSMutableDictionary dictionaryWithDictionary:request.userInfo];
    NSData *responseData = request.responseData;
    if ([responseData length] > 0) {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kUpdateCustKKServiceCompletedNotification object:self userInfo:userInfoDict];
...
- (void)updateCustKKService:(CustKKService *)aCustKKService
{
    ...
    NSString *payload = nil;
    if (self.useJSON) {
        payload = [service getJSONForUpdateRequest:aCustKKService error:&error];
    }
    else {
        payload = [service getXMLForUpdateRequest:aCustKKService error:&error];
    }
    ...
    if (error) {
        ...
        ODataQuery *query = [[ODataQuery alloc] initWithURL:aCustKKService.baseUrl];
    	NSDictionary *userInfoDict = @{kResponseParentItem : aCustKKService , kRequestUserInfoCompletedNotification : kUpdateCustKKServiceCompletedNotification};
    	[connectivityHelper executeUpdateAsyncRequestWithQuery:query andBody:payload andCSRFData:csrfData andRequestDelegate:self andDidFinishSelector:@selector(updateCustKKServiceCompleted:) andUserInfo:userInfoDict andEtag:aCustKKService.etag];
    }
}

#pragma mark Delete

- (void)deleteCustKKServiceCompleted:(id <Requesting>)request
{
    NSMutableDictionary *userInfoDict = [NSMutableDictionary dictionaryWithDictionary:request.userInfo];
    [[NSNotificationCenter defaultCenter] postNotificationName:kDeleteCustKKServiceCompletedNotification object:self userInfo:userInfoDict];   
}
...
- (void)deleteCustKKService:(CustKKService *)aCustKKService 
{
	[self loadCSRFData];
    ODataQuery *query = [[ODataQuery alloc] initWithURL:aCustKKService.baseUrl];
    NSDictionary *userInfoDict = @{kResponseParentItem : aCustKKService , kRequestUserInfoCompletedNotification : kDeleteCustKKServiceCompletedNotification};
    [connectivityHelper executeDeleteAsyncRequestWithQuery:query andCSRFData:csrfData andRequestDelegate:self andDidFinishSelector:@selector(deleteCustKKServiceCompleted:) andUserInfo:userInfoDict andEtag:aCustKKService.etag];
}

#pragma mark - ET_CARDSet methods
#pragma mark Read

- (void)loadET_CARDSetCompleted:(id <Requesting>)request
{
    NSDictionary *userInfoDict;
    NSError *error;
    NSMutableArray *items = [service getET_CARDSetWithData:request.responseData error:&error];
    if (error) {
    	userInfoDict = @{kParsingError: error};
    }
    else {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kLoadET_CARDSetCompletedNotification object:self userInfo:userInfoDict];
}

- (void)loadET_CARDSet
{
    ODataQuery *query = service.ET_CARDSetQuery;
    NSDictionary *userInfoDict = @{kRequestUserInfoCompletedNotification : kLoadET_CARDSetCompletedNotification};
    [connectivityHelper executeBasicAsyncRequestWithQuery:query andRequestDelegate:self andDidFinishSelector:@selector(loadET_CARDSetCompleted:) andUserInfo:userInfoDict];
}


- (void)loadET_CARDSCompleted:(id <Requesting>)request
{
    NSMutableDictionary *userInfoDict = [NSMutableDictionary dictionaryWithDictionary:request.userInfo];
    NSError *error;
    ET_CARDS *item = [ET_CARDS parseET_CARDSEntryWithData:request.responseData error:&error];
    if (error) {
    	userInfoDict[kParsingError] = error;
    }
    else {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kLoadET_CARDSCompletedNotification object:self userInfo:userInfoDict];
}

- (void)loadET_CARDS:(ET_CARDS *)aET_CARDS
{
    ODataQuery *query = [[ODataQuery alloc] initWithURL:aET_CARDS.baseUrl];
    NSDictionary *userInfoDict = @{kResponseParentItem : aET_CARDS, kRequestUserInfoCompletedNotification : kLoadET_CARDSCompletedNotification};
    [connectivityHelper executeBasicAsyncRequestWithQuery:query andRequestDelegate:self andDidFinishSelector:@selector(loadET_CARDSCompleted:) andUserInfo:userInfoDict];
}

#pragma mark Create

- (void)createET_CARDSCompleted:(id <Requesting>)request
{
    NSMutableDictionary *userInfoDict = [NSMutableDictionary dictionaryWithDictionary:request.userInfo];
    NSData *responseData = request.responseData;
    if ([responseData length] > 0) {
        NSError *error;
        ET_CARDS *item = [ET_CARDS parseET_CARDSEntryWithData:responseData error:&error];
        if (error) {
        	userInfoDict[kParsingError] = error;
        }
        else{
...
- (void)createET_CARDS:(ET_CARDS *)aET_CARDS
{
    ...
    NSString *payload = nil;
    if (self.useJSON) {
        payload = [service getJSONForCreateRequest:aET_CARDS error:&error];
    }
    else {
        payload = [service getXMLForCreateRequest:aET_CARDS error:&error];
    }
	...
	if (error) {
        ...
        NSDictionary *userInfoDict = @{kResponseParentItem : aET_CARDS , kRequestUserInfoCompletedNotification : kCreateET_CARDSCompletedNotification};
    	[connectivityHelper executeCreateAsyncRequestWithQuery:service.ET_CARDSetQuery andBody:payload andCSRFData:csrfData andRequestDelegate:self andDidFinishSelector:@selector(createET_CARDSCompleted:) andUserInfo:userInfoDict];
    }
}

#pragma mark Update

- (void)updateET_CARDSCompleted:(id <Requesting>)request
{
    NSMutableDictionary *userInfoDict = [NSMutableDictionary dictionaryWithDictionary:request.userInfo];
    NSData *responseData = request.responseData;
    if ([responseData length] > 0) {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kUpdateET_CARDSCompletedNotification object:self userInfo:userInfoDict];
...
- (void)updateET_CARDS:(ET_CARDS *)aET_CARDS
{
    ...
    NSString *payload = nil;
    if (self.useJSON) {
        payload = [service getJSONForUpdateRequest:aET_CARDS error:&error];
    }
    else {
        payload = [service getXMLForUpdateRequest:aET_CARDS error:&error];
    }
    ...
    if (error) {
        ...
        ODataQuery *query = [[ODataQuery alloc] initWithURL:aET_CARDS.baseUrl];
    	NSDictionary *userInfoDict = @{kResponseParentItem : aET_CARDS , kRequestUserInfoCompletedNotification : kUpdateET_CARDSCompletedNotification};
    	[connectivityHelper executeUpdateAsyncRequestWithQuery:query andBody:payload andCSRFData:csrfData andRequestDelegate:self andDidFinishSelector:@selector(updateET_CARDSCompleted:) andUserInfo:userInfoDict andEtag:aET_CARDS.etag];
    }
}

#pragma mark Delete

- (void)deleteET_CARDSCompleted:(id <Requesting>)request
{
    NSMutableDictionary *userInfoDict = [NSMutableDictionary dictionaryWithDictionary:request.userInfo];
    [[NSNotificationCenter defaultCenter] postNotificationName:kDeleteET_CARDSCompletedNotification object:self userInfo:userInfoDict];   
}
...
- (void)deleteET_CARDS:(ET_CARDS *)aET_CARDS 
{
	[self loadCSRFData];
    ODataQuery *query = [[ODataQuery alloc] initWithURL:aET_CARDS.baseUrl];
    NSDictionary *userInfoDict = @{kResponseParentItem : aET_CARDS , kRequestUserInfoCompletedNotification : kDeleteET_CARDSCompletedNotification};
    [connectivityHelper executeDeleteAsyncRequestWithQuery:query andCSRFData:csrfData andRequestDelegate:self andDidFinishSelector:@selector(deleteET_CARDSCompleted:) andUserInfo:userInfoDict andEtag:aET_CARDS.etag];
}
